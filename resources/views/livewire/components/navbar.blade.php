...
<header id="mainNavbar"
    class="{{ $isHome ? 'absolute' : 'fixed' }} top-0 left-0 w-full flex items-center justify-between px-6 md:px-10 py-4 z-50 transition-all duration-300 ease-in-out {{ $isHome ? 'bg-transparent text-white' : 'bg-white text-gray-900 shadow-sm' }}">
    <div class="text-xl md:text-2xl font-bold transition-colors duration-300 {{ $isHome ? 'text-white' : 'text-emerald-600' }}"
        id="brandName"> Wisata Desa Kunjir </div>
    ...
    </nav> <button id="mobileMenuBtn" class="md:hidden focus:outline-none z-50" aria-label="Toggle menu"> <svg
            id="menuIcon" class="w-8 h-8 {{ $isHome ? 'text-white' : 'text-gray-900' }} transition-colors duration-300"
